...
module.exports = exports = stateMachinePlugin = (schema, init_with) ->
  ...
  schema.methods.fire = (event, err_callback) ->
    ...
    if !paths.hasOwnProperty(this.state)
      return err_call(err_callback, new Error("Invalid state '#{this.state}'"))
    ...
    if new_state
      if !paths.hasOwnProperty(new_state)
        return err_call(err_callback, new Error("Unable to change state; '#{this.state}' -> '#{new_state}' not valid transition"))

      this.state = new_state
      this.save (err) ->
        return err_call(err_callback, err)
